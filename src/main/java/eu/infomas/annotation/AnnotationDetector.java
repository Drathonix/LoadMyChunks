 ...
 * @since eu.infomas.annotation-detector 3.0.0
 */
public final class AnnotationDetector {
    ...
    private final eu.infomas.annotation.ClassFileBuffer cpBuffer = new eu.infomas.annotation.ClassFileBuffer();
    // the eu.infomas.annotation types to report, see {@link #annotations()}
    ...
    public void detect() throws IOException {
        detect(new eu.infomas.annotation.ClassFileIterator());
    }
    ...
    public void detect(final String... packageNames) throws IOException {
        ...
        if (!files.isEmpty()) {
            ...
            detect(new eu.infomas.annotation.ClassFileIterator(files.toArray(new File[0]), pkgNameFilter));
        }
    }
    ...
    private boolean hasCafebabe(final eu.infomas.annotation.ClassFileBuffer buffer) throws IOException {
    ...
    private void readAnnotations(final DataInput di, final char reporterType)
        ...
        final int count = di.readUnsignedShort();
        if (DEBUG) {
            print("eu.infomas.annotation count (%s) = %d", reporterType, count);
        }
        for (int i = 0; i < count; ++i) {
    ...
    private String readAnnotation(final DataInput di) throws IOException {
        ...
        final int count = di.readUnsignedShort();
        if (DEBUG) {
            print("eu.infomas.annotation elements count: %d", count);
        }
        for (int i = 0; i < count; ++i) {
            if (DEBUG) {
    ...
    private void readAnnotationElementValue(final DataInput di) throws IOException {
        ...
        switch (tag) {
            ...
            case ARRAY:
                ...
                break;
            default:
                throw new ClassFormatError("Not a valid eu.infomas.annotation element type tag: 0x" +
